...
def test_topic_creation(db):
    ...
    topic = mixer.blend('group_projects.Topic', title='Test Topic')
...
def test_topic_title_max_length(db):
    ...
    topic = mixer.blend('group_projects.Topic', title='A' * 101)
...
def test_goal_creation(db):
    ...
    goal = mixer.blend('group_projects.Goal', title='Test Goal', points=3)
...
def test_goal_points_min_value(db):
    ...
    goal = mixer.blend('group_projects.Goal', points=0)
...
def test_goal_points_max_value(db):
    ...
    goal = mixer.blend('group_projects.Goal', points=6)
    with pytest.raises(ValidationError) as err:
        goal.full_clean()
...
def test_goal_description_max_length(db):
    ...
    goal = mixer.blend('group_projects.Goal', description='A' * 401)
    with pytest.raises(ValidationError):
        goal.full_clean()
...
def test_group_creation(db):
    ...
    topic = mixer.blend('group_projects.Topic')
    group = mixer.blend('group_projects.GroupProject', 
                       name='Test Group',
                       topic=topic,
...
# ============== GROUPGOAL MODEL TESTS ==============

def test_groupgoal_creation(db):
    """Test che una relazione gruppo-goal venga creata correttamente"""
    group = mixer.blend('group_projects.GroupProject')
    goal = mixer.blend('group_projects.Goal')
    group_goal = mixer.blend('group_projects.GroupGoal', group=group, goal=goal, complete=False)
...
def test_groupgoal_complete_default_false(db):
    """Test che il campo complete sia False di default"""
    group_goal = mixer.blend('group_projects.GroupGoal')
    assert group_goal.complete is False


def test_groupgoal_can_be_completed(db):
    """Test che un goal possa essere marcato come completato"""
    group_goal = mixer.blend('group_projects.GroupGoal', complete=True)
